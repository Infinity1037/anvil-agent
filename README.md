...
- `/` 弹出命令补全（按前缀过滤：help / status / context / compact / effort / perm / clear / resume / expand / exit）
- `@` 弹出工作区文件补全
- `?` 或 `F1` 打开键位说明（输入框为空时）
...
命令行：`anvil --workspace .` 进入全屏对话；后面可以跟一句首条任务。`--once` 表示只跑这一次。`--plain` 关闭全屏 TUI。`--effort low|high|max` 覆盖启动时的思考强度。`--continue` 恢复本工作区最近一次会话。`/status` `/context` `/compact` `/effort` `/clear` `/resume` `/exit` 是会话内命令。
